   ...
   /*  function handleSubmit(event) {
        ...
        fetch(url, {
            ...
            instances.forEach(instance => {
                let courseId = instance.courseId;
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        if (data && data.Data) {
                            ...
                            console.log("Courses DATATA>>>>>>>>>>>>>>>>.", data.Data)
                        }
                        else {
                            console.error("Invalid data format received from the backend", data);
                        }
            ...
            const fetchCourses = async () => {
                const courseData = await Promise.all(
                    instances.map(async (instance) => {
                        const response = await fetch(`/courses/${instance.courseId}`);
    ...
    let handleSubmit = (event) => {
        ...
        fetch(`/instances/${year}/${selectValue}`)
            .then((res) => res.json())
            .then((data) => {
                if (data && data.Data) {
                    setInstances(data.Data);
                        ...
                        {instances.map((inst, index) => (
                            <tr key={index}>
                                ...
                                <td>
                                    ...
                                    <span><button className="btn btn-outline-secondary" type="button"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-trash3" viewBox="0 0 16 16">
